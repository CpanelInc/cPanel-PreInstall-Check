...
GetOptions ( 'nocolor' => sub{ $nocolor = 1 }, 
...
if ($help == 1) {
    ...
    print "--force-install : Grabs the cPanel Installer and Forces it to run for RHEL 5 based machines\n"; 
    print "========================================================================================================================\n";
    exit 0;  
}
if ($nocolor == 1) {
...
if ($install == 1) {
    if ($rhel5) {       
        print "${yellow}[WARN] * $rhel (Linux) detected!${NC}\n";
        print "[INFO] * In order to take full advantage of all the features provided by cPanel & WHM, such as multiple SSL Certificates on a single IPv4 Address, we highly recommend you use a $rhelrec 6.x OS\n";
        print "[INFO] * You can force the install on $rhel using the --forceinstall option.\n";     
    }  
    elsif ($rhel6) {
        &cpinstall;
    }
